...
# Loop through all items and insert meta data into dynamo
def loop_through_response(response):
    for item in response['Items']:
        matched_terms = fetch_descriptions(item)
        update_item(item['property_id'], matched_terms, table)
...
def item_is_new(description, item, year_cut_off ):
    ...
    if new:
        return True
    completed_in_pref_format = re.findall('.*' + item+ '[a-z\s\(\,\+\&\)]{0,}(20[0-9][0-9]|19[0-9][0-9]|[0-9]{2})', description)
    if completed_in_pref_format:
        return CURRENT_YEAR - int(completed_in_pref_format[0]) <= year_cut_off 
    completed_in_secondary = re.findall('.*' + item+ '[a-z\s\(\,\+\&\)]{0,}([0-9]{2})', description)
    ...
    years_old = re.findall('.*' + item+ '[a-z\s\(\,\+\&\)]{0,}([0-9]|[0-9]{2})[a-z0-9\s\(\,\+\&\)]{0,}(years|yrs|old)', description)
    if years_old:
...
def fetch_description_metadata(item):
    ...
    max_ages = [            6,             6,       10,      4,         7,          7,      7,      7,         10,           20]
    ...
    kitchen_features = ['ceramic tile backsplash', 'maytag', 'whirlpool', 'granite counter', 'tile counter', 'slate flooring', 'stainless steel appliances']
    ...
    walls = ['freshly painted', 'large windows', 'vinyl windows', 'natural light']
    for i in range(len(walls)):
        match = regex_handler(description, walls[i])
        if match:
            items.append( walls[i].replace(" ", "_"))         

    general = ['ocean breeze', 'ocean view', 'ocean-breeze', 'balcony', 'fireplace', 'basement', 'attic', 'cash flow', 'auction', 'porch', 'recent cosmetic upgrades']
    ...
    for i in range(len(percentage_calculations)):
        tag = get_percentage_description(description, percentage_calculations[i], 5.0, 3.0)
        if tag:
            items.append(tag)
    return items